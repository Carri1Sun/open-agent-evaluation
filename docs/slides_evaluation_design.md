...
本文档定义 general agent 在 slides/PPT 生成任务上的评测方法。评测目标是把可客观验证的部分自动化，并把主观判断拆成稳定、可复用、可解释的 grader；人工审美可以作为开放性评审的一部分进入 rubric。

## 1. 任务边界

仓库的边界是评测 agent 产出的结果；agent 执行由外部系统完成。一次 slides 评测接收：
...
它证明行为信号存在；产物正确性需要结合 artifact、content 或 visual grader 判断。
...
本仓库保持 LLM 供应商无关，`llm_judge` 默认生成评审 payload；实际执行可通过 `command` 接入 LLM、PI agent、OpenCode agent 或人工系统。
...
这种协议让更强的评审 agent 可以通过配置或外部命令接入，核心流水线保持稳定。
...
- PPTX 静态检查负责结构性问题；视觉和事实判断由 external/rubric 扩展覆盖。
- 数据型 slides 需要 case-specific scripted grader，还要校验 chart 背后的数据口径。
